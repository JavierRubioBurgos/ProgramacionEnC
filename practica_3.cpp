...
/*Subprogramas*/
  ...
  /*Para sumar días de la semana cíclicamente*/
  ...
  /*Para calcular el día de la semana que corresponde*/
   ...
   TipoDia DiaDeLaSemana ( TipoFecha fecha ) {
	   ...
	   IncreAnnos = A - AnnoReferencia;/* Incremento de días por años completos desde año referencia */
    ...
    for (int N=1602; N<A; N++) {
      if (bisiesto (N)) {
	   ...
	   if ( bisiesto (A) && /* añadir un día si la fecha a evaluar cae en un ano bisiesto despues del día 29 de febrero */
		( M > Febrero )) {
		   IncreDias ++;
		   }
...
int main() {
  ...
  fecha.dia = 1;							/* iniciar el primer día de la fecha para poder determinar cual es el primer día del mes*/

 /*Preguntar datos  y determinar información necesaria a la realización del calendario */
